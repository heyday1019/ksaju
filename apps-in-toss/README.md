...
| `tarot/*.webp` | 메인 레포 `public/tarot/` 78장 (79MB PNG) | 3.60MB |

타로 덱은 400px 로 줄인다. 화면에서 가장 큰 '오늘의 타로' 한 장이 CSS 160px 이라
3배 해상도 기기에서 480px 이면 충분하고, 400px 을 480 으로 확대해도 원본과 구분되지 않는다.
**초기 로딩에는 영향이 없다** — Vite 가 장당 개별 파일로 내보내고 화면에 뜬 카드만 받아간다
(사주 화면에서 받는 타로 이미지 0장, 전 화면을 돌아도 78장 중 4장). 늘어나는 건
`.ait` 최초 내려받기 용량뿐이다(4.3MB → 7.8MB).
